...
const UserContactTable = ({ setSelectedUserContact, searchTerm = "" }) => {
  const userContacts = useMemo(() => {
    const data = JSON.parse(sessionStorage.getItem(USER_CONTACT_LIST) ?? "[]");
    const treated = treatContactData(data);
    ...
    return treated.filter(contact => 
      contact.name.toLowerCase().includes(term) ||
      contact.email.toLowerCase().includes(term) ||
      (contact.localization && contact.localization.toLowerCase().includes(term)) ||
    ...
    <Table hover className="mb-0 admin-table">
      <thead className="d-none d-md-table-header-group">
        <tr>
          ...
          <th className="text-muted small fw-semibold text-uppercase d-none d-lg-table-cell">Localização</th>
            ...
            >
              ...
              <td>
                <div className="d-flex align-items-center">
                  <div className="admin-avatar me-2 me-md-3">
                    {getInitials(name)}
                  </div>
                  <div className="flex-grow-1">
                    <div className="fw-semibold text-dark">{name}</div>
                    <div className="text-muted small d-md-none">{email}</div>
                    <div className="text-muted small d-none d-md-block">{contact || 'Sem telefone'}</div>
                  </div>
                </div>
              </td>
              <td className="d-none d-md-table-cell">
